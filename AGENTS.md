...
# Текущее состояние (до авторизации)

## База данных

- **PostgreSQL** через `@prisma/adapter-pg`
- **5 моделей:** `Category`, `Product`, `Customer`, `Order`, `OrderItem`
- **2 enum:** `PaymentMethod (CARD | CASH | BONUS)`, `OrderStatus (NEW | PROCESSING | COMPLETED | CANCELLED)`
- **Customer** — без полей `passwordHash`, `role`, `isActive`, `totpSecret`, `refreshTokens`
- **Одна миграция:** `20260713075951_init`
- seed: 7 категорий, 23 товара, 5 клиентов, 7 заказов

## API-сервер

- **Express 5**, монтирован на `/api` (не `/api/v1`)
- **Нет** `cookie-parser`, `jose`
- **CORS** — без `origin` и `credentials` (голый `app.use(cors())`)
- **Логгер** — pino с pino-pretty в dev
- **errorHandler** — ZodError → 400, P2002 → 409, кастомные `{status}` → по статусу, >=500 скрывает сообщение

## Фронтенд

- **React + Vite + Tailwind + React Router 7**
- **`api/client.ts`** — только `apiGet<T>()`, нет POST/PUT/DELETE, нет `Authorization`, нет `credentials: "include"`
- **`App.tsx`** — 4 роута: `/`, `/category/:slug`, `/product/:id` (ComingSoon), `/account` (ComingSoon)
- **Нет:** страницы логина, админ-роутинга, `RequireRole`, auth-контекста, токен-менеджмента

## Инфраструктура

- **Нет Docker/`docker-compose`** — dev полагается на local PostgreSQL
- **Нет `.github/workflows/`** — CI не настроен
- **`TEST_DATABASE_URL`** — переменная для интеграционных тестов
