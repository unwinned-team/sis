...
## Конвертация старых картинок в WebP

`server/prisma/backfill-images.cjs` — разовая операция. Новые аплоады админки
конвертируются на лету (`server/src/lib/processImage.ts`), скрипт нужен только
для файлов, залитых до этого.

Что он делает: png/jpg/jpeg в каталоге → WebP 1600px q80 (`fit: inside`, без
апскейла, EXIF-поворот применяется), обновляет `imageUrl` в `product`,
`productVariant` и `category` **одной транзакцией**, копирует исходники в
`<dir>/.backup-<timestamp>/` и удаляет их — но только после коммита транзакции.

### Аплоады админки (`server/uploads`)

```bash
cd /srv/ice-shop

# 1. Бэкап БД — скрипт в неё пишет
sudo -u postgres pg_dump ice_shop > ~/ice_shop_before_webp.sql

# 2. Сухой прогон: печатает список, ничего не меняет
node server/prisma/backfill-images.cjs server/uploads /uploads --dry-run

# 3. Боевой прогон
node server/prisma/backfill-images.cjs server/uploads /uploads

# 4. Открыть витрину и проверить товары, у которых были фото

# 5. Только после проверки
rm -rf server/uploads/.backup-*
```

Останавливать сервер не нужно: до коммита транзакции БД ссылается на исходники,
и они ещё на диске; после коммита — на `.webp`, которые уже готовы. Битого окна нет.

Запускать из корня репозитория (первый аргумент — обычный путь, он резолвится от
cwd) и **от того же пользователя, под которым работает pm2** — иначе новые файлы
получат чужого владельца.

### Фото каталога (`web/public/images/products`)

Другой префикс, и после него обязательна пересборка фронта — файлы попадают в
бандл на сборке. Литералы `"/images/products/*.jpg"` в `prisma/catalog.cjs`
скрипт переписывает сам.
...
Изменения в `catalog.cjs` и новые `.webp` — закоммитить; `.backup-*` покрыт
`.gitignore`.
...
| Флаг | Что делает |
|---|---|
| `--dry-run` | печатает `would convert ...`, не создаёт ни бэкапа, ни файлов |
| `--no-db` | конвертирует файлы и **удаляет исходники, не трогая БД** |

`--no-db` на каталоге, на который ссылается БД, оставит все `imageUrl` битыми.
Применять только к каталогам вне БД.

Второй аргумент — префикс ровно в том виде, в каком URL лежит в базе
(`/uploads` или `/images/products`). Ошибётесь — файлы сконвертируются, а `imageUrl`
не совпадёт ни с одной строкой: `db:` в выводе не будет, и это единственный признак.

### Ограничения

- **Больше ~100 файлов за прогон** — все `updateMany` идут в одной интерактивной
  транзакции Prisma с дефолтным лимитом 5 секунд. На большом каталоге прогон падает
  с `P2028`: файлы сконвертированы, исходники целы, БД не тронута. Лечится повтором
  меньшими порциями (временно вынести часть файлов) или явным `timeout` в
  `$transaction`.
- **Одинаковые имена с разными расширениями** (`x.jpg` + `x.png`, или уже лежащий
  рядом `x.webp`) — оба уходят в один `x.webp`, второй перезаписывает первый, и
  готовый `.webp` затирается без бэкапа. Перед прогоном проверить:
  `ls <dir> | sed 's/\.[^.]*$//' | sort | uniq -d`
- Скрипт читает только верхний уровень каталога, во вложенные не заходит.
