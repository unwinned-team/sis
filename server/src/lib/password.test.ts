...
test("verify supports different parameters parsed from the stored hash", async () => {
  ...
  const digest = scryptSync("legacy-password", salt, 64, { N: 16384, r: 8, p: 1 });
...
test("malformed stored hashes are rejected without throwing", async () => {
  for (const stored of ["", "not-a-hash", "bcrypt$10$abc$def", "scrypt$0$8$1$aaaa$bbbb", "scrypt$32768$8$1"]) {
    assert.equal(await verifyPassword("password", stored), false, `stored "${stored}"`);
  }
});

test("DUMMY_HASH is well-formed and never matches", async () => {
  assert.match(DUMMY_HASH, /^scrypt\$32768\$8\$1\$[A-Za-z0-9_-]+\$[A-Za-z0-9_-]+$/);
